...
The Vehicle Controller is a special :ref:`type of physics object <game_engine-physics-types>` that the Physics Engine (bullet) recognizes.
...
You can check Kester Maddock's approach to vehicle simulation `here <http://tinyurl.com/ydfb7lm>`__. It includes some common problems, workarounds and tips and tricks.
...
^^^^^^^^^^^^^^^^^^^^^^^
This is done in 2 parts and it should be modeled according to the desired behavior. You should think of your gameplay and research appropriate functions for the input. For instance, can the vehicle reverse? jump? drift? does it turn slowly? How much time does it take to brake or get to full speed?
The first part is **response to keys**. Whenever the player presses a key, you should set a value accordingly, such as increase acceleration. Example:
...
Example
-------
`demo_file.zip <https://dl.dropboxusercontent.com/u/3226675/blender/vehicle_controller_demo.zip>`__ (last update 9 September 2014)
